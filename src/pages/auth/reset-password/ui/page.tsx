  ...
  return (
    <Form onChange={handleFormChande} method="post" action="/reset" aria-describedby="form-error" className="w-full sm:w-90">
      <fieldset disabled={navigation.formAction === '/reset'}>
        ...
        {actionData?.errors?.email ? <p id="email-error" role="alert" className="mt-1 text-2xs text-center text-red-400">{actionData?.errors?.email}</p> : null}
        <button className="w-full mt-5 p-3.5 rounded-xl bg-black text-white cursor-pointer outline-offset-3">Reset</button>
      </fieldset>
      {actionData?.errors?.message ? <p id="form-error" role="alert" className="mt-5 text-2xs text-center text-red-400">{actionData?.errors.message}</p> : null}
