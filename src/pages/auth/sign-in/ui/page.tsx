    ...
    <Form onChange={handleFormChande} method="post" aria-describedby="form-error" className="w-full sm:w-90">
      <fieldset disabled={navigation.formAction === '/signin'}>
        ...
        <label htmlFor="email" className="block mt-8 relative">
          <input id="email" name="email" type="email" placeholder="name@company.com" aria-describedby="email-error" className="peer w-full p-3.5 pl-11 rounded-xl bg-[#f5f5f5] placeholder:text-[#C2C3CB] data-[error=true]:outline-2" data-error={isError && actionData?.errors?.email?.length ? true : false} required />
        ...
        {actionData?.errors?.email ? <p id="email-error" role="alert" className="mt-1 text-2xs text-center">{actionData?.errors?.email.join( ', ' )}</p> : null}
        <label htmlFor="password" className="block mt-5 relative">
          <input id="password" name="password" type="password" placeholder="•••••••••" aria-describedby="password-error" className="peer w-full p-3.5 pl-11 rounded-xl bg-[#f5f5f5] placeholder:text-[#C2C3CB] data-[error=true]:outline-2" data-error={isError && actionData?.errors?.password?.length ? true : false} required />
          <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 fill-[#C2C3CB] peer-focus:fill-black absolute top-1/2 left-4 -translate-y-1/2" aria-hidden="true">
        ...
        {actionData?.errors?.password ? <p id="password-error" role="alert" className="mt-1 text-2xs text-center">{actionData?.errors.password.join( ', ' )}</p> : null }
